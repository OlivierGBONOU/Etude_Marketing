...
st.set_page_config(
    page_title="Rapport - Services de ménage pour étudiants ENSEA",
    page_icon="🧹",
    layout="wide"
)
...
color_map = {'Pas du tout intéressé(e)': 'darkred', 'Plutôt pas intéressé(e)': 'orangered', 'Indécis(e)': 'gold', 'Plutôt intéressé(e)': 'lightgreen', 'Très intéressé(e)': 'green'}
...
with col1:
    ...
    fig = px.scatter(df, x='temps_menage_hebdo_num', y='difficulte_num', 
                     ...
                     labels={'temps_menage_hebdo_num': 'Temps hebdomadaire', 
...
st.markdown("""
Les résultats de l’analyse indiquent que l’intérêt des étudiants de l'ENSEA pour un service de ménage est globalement positif, quoique modéré. L'échantillon, composé majoritairement d'étudiants en résidence universitaire et en première année d'Analyste Statisticien, montre que :
- Les hommes et les étudiants en résidence universitaire sont particulièrement intéressés par le service.
- Le manque de temps et de motivation, notamment pendant les périodes d'examens et de projets, constitue le principal frein à l'utilisation du service.
- Le budget mensuel consacré au ménage est limité (inférieur à 5000 FCFA pour la majorité des étudiants), et les prestations préférées sont la lessive, le ménage complet et des interventions sur des zones spécifiques.
- La préférence s'oriente vers un service hebdomadaire, sous forme d'abonnement, avec la présence du prestataire et sans remise de clé.
- La gestion du ménage est perçue différemment selon le type de logement, les étudiants en colocation ou en résidence universitaire ayant plus de difficultés à concilier ménage et études que ceux en appartement ou vivant chez leurs parents.

**Recommandations :**
1. **Offre flexible adaptée aux pics de demande :** Proposer une offre modulable qui permet de réserver des prestations de ménage supplémentaires lors des périodes d’examens ou de projets intensifs.
2. **Ciblage des résidences universitaires et colocations :** Développer des formules spécifiques pour les étudiants vivant en résidence universitaire ou en colocation, qui rencontrent plus de difficultés à gérer le ménage.
3. **Focus sur les prestations préférées :** Mettre en avant des services de lessive et de ménage complet, et envisager des offres combinées incluant l’entretien de zones spécifiques.
4. **Service d’abonnement hebdomadaire avec présence du prestataire :** Proposer un abonnement hebdomadaire où le prestataire est présent pendant toute la durée du ménage, afin de rassurer les utilisateurs tout en respectant leur souhait de ne pas confier leurs clés.
5. **Tarification adaptée aux budgets étudiants :** Développer une tarification compétitive, avec des forfaits mensuels adaptés aux budgets limités, en particulier pour ceux consacrant moins de 5000 FCFA au ménage.
""")
